...
def tejer_categorias(request):
    t=el_tejedor(url='http://books.toscrape.com/')
    n_categorias                =   t.tejer_categorias()
    response                    =   HttpResponse(status=205)
    response['n_categorias']    =   n_categorias
    msg                         =   f'categorias encontradas {n_categorias}'
    return HttpResponse(msg, content_type='text/plain')
    return response

def tejer_libros_categoria(request,url_categoria):
    print (url_categoria.replace(',','/'))
    msg                         =   f'categorias encontradas'
    return HttpResponse(msg, content_type='text/plain')
    return response
...
class el_tejedor():
    ...
    def tejer_categorias(self):
        categorias  =   self.traer_categorias()
        for c in categorias:
    ...
    def tejer_libros_categoria(self, url):
        for l in self.traer_libros_categorias(url):
